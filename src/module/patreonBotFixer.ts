    ...
    function implementationOne() {
        ...
        */
        client.on(Events.GuildAuditLogEntryCreate, async (entry, guild) => {
            if (entry.executor?.id !== PATREON_DISCORD_BOT)
                return;

            if (entry.action !== AuditLogEvent.MemberRoleUpdate)
                return;

            if (entry.changes.length !== 1) // Role was removed rather than added
                return

            if (entry.changes[0].key !== '$remove') // On role removal
                return

            if (entry.changes[0].old?.length !== 1) // Ensure that a role was removed
                return
        ...
        function schedule(member: GuildMember, roles: Role[]) {
            console.log(
                `Scheduled Patreon fix for ${member.user.username}'s in guild ${member.guild.name} (${member.guild.id}).`
            );
        ...
        function unschedule(member: GuildMember) {
            console.log(
                `Completed scheduled Patreon fix for ${member.user.username}'s in guild ${member.guild.name} (${member.guild.id}).`
            );
